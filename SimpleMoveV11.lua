...
local function safeComputePath(startPos, endPos)
    for i = 1, 3 do -- tente 3 fois max
        local path = PathfindingService:CreatePath({
            AgentRadius = 1.5,      -- plus petit : plus tolérant
            AgentHeight = 6,        -- un peu plus grand que le perso
            AgentCanJump = true,
            AgentCanClimb = true,
            WaypointSpacing = 1,    -- plus petit = plus précis
            Costs = { Water = 1 },  -- tolère tous les types de surface
        })
        path:ComputeAsync(startPos, endPos)
        if path.Status == Enum.PathStatus.Success then
            return path
        end
        print(("⚠️ Tentative %d/3 échouée, retry..."):format(i))
        task.wait(0.1)
    end
    return nil
end
...
print("Auto Move V11 chargé! Pathfinding tolérant pour terrains irréguliers")