    ...
    if target and target:FindFirstChild("HumanoidRootPart") then
        ...
        if char and char:FindFirstChild("Humanoid") then
            ...
            if distance < 5 then
                ...
                -- Simulation de pression de la touche "2"
                local success, result = pcall(function()
                    -- Essayer d'utiliser l'API d'input si disponible
                    if UserInputService then
                        UserInputService:FireInputEvent(Enum.UserInputType.Keyboard, Enum.KeyCode.Two, true)
                        task.wait(0.1)
                        UserInputService:FireInputEvent(Enum.UserInputType.Keyboard, Enum.KeyCode.Two, false)
                    end
                end)
                
                if not success then
                    print("⚠️ Attaque simulée (API d'input non disponible)")
                end
            end
        end
    end
...
print("Auto Move V20 chargé! Anti-collage + attaque automatique")
