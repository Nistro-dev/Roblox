        ...
        if isMoving and humanoid and root then
            ...
            if target and target:FindFirstChild("HumanoidRootPart") then
                ...
                if hit then
                    ...
                    local isWall = math.abs(normal.Y) < 0.2

                    if isWall then
                        print(("🧱 Mur détecté : %s distance: %.1f"):format(hitPart.Name, hit.Distance))
                        -- Petit recul avant le contournement
                        humanoid:Move(-root.CFrame.LookVector, false)
                        task.wait(0.2)
                        
                        -- Contournement intelligent avec Cross product
                        local cross = root.CFrame.LookVector:Cross(hit.Normal)
                        local strafeDir = cross.Y > 0 and root.CFrame.RightVector or -root.CFrame.RightVector
                        humanoid:Move(Vector3.new(strafeDir.X * 2, 0, strafeDir.Z * 2), false)
                        task.wait(0.6)
                        -- Puis reprendre
                        humanoid:Move(Vector3.new(direction.X, 0, direction.Z), false)
                    else
                        -- Ce n'est pas un mur (rocher, pente, obstacle irrégulier)
                        print(("🪨 Obstacle non mural : %s matériau: %s"):format(hitPart.Name, tostring(hitPart.Material)))
                        humanoid.Jump = true
                        task.wait(0.3)
                        humanoid:Move(Vector3.new(direction.X, 0, direction.Z), false)
                    end
...
print("Auto Move V17 chargé! Détection de blocage + contournement étendu")
