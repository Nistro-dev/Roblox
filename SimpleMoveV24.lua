    ...
    RunService.RenderStepped:Connect(function(dt)
        if isMoving and humanoid and root then
            ...
            if target and target:FindFirstChild("HumanoidRootPart") then
                ...
                -- Détection d'obstacle pour saut amélioré
                ...
                -- Descente automatique / pas bloqué sur rebord
                local downCheck = workspace:Raycast(root.Position + Vector3.new(0,2,0), Vector3.new(0, -6, 0), rayParams)
                if downCheck then
                    if downCheck.Distance > 5 then
                        -- sol plus bas, on avance
                        humanoid:Move(Vector3.new(toTarget.X, 0, toTarget.Z), false)
                        print("⬇️ Descente automatique détectée")
                    elseif downCheck.Distance < 1 then
                        -- très proche sol ou rebord, on recule un peu
