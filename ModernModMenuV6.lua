...
function createGUI()
    ...
    screenGui.Name = "ModernModMenuV6"
...
-- Fonction pour appliquer le god mode avec valeurs réalistes
function applyGodMode()
    ...
    if char and char:FindFirstChild("Humanoid") then
        ...
        -- God mode avec valeurs réalistes
        char.Humanoid.MaxHealth = 999999
        char.Humanoid.Health = 999999
        
        print("🛡️ God Mode appliqué avec valeurs réalistes (999999 HP)")
    end
end
...
function exploreGameStructure()
    ...
    print("\n📦 EXPLORATION REPLICATEDSTORAGE:")
    local replicatedStorage = game:GetService("ReplicatedStorage")
    if replicatedStorage then
        local rsChildren = replicatedStorage:GetChildren()
        print("  📁 Enfants de ReplicatedStorage (" .. #rsChildren .. "):")
        for _, child in ipairs(rsChildren) do
            ...
            if child:IsA("Folder") then
                ...
                for i = 1, math.min(5, #folderChildren) do
                    local item = folderChildren[i]
                    print("        📄 " .. item.Name .. " (" .. item.ClassName .. ")")
                end
            end
    ...
    print("\n🗄️ EXPLORATION SERVERSTORAGE:")
    local serverStorage = game:GetService("ServerStorage")
    if serverStorage then
        local ssChildren = serverStorage:GetChildren()
        print("  📁 Enfants de ServerStorage (" .. #ssChildren .. "):")
        for _, child in ipairs(ssChildren) do
            print("    📄 " .. child.Name .. " (" .. child.ClassName .. ")")
        end
    end
    
    -- Explorer StarterGui
    print("\n🖥️ EXPLORATION STARTERGUI:")
    local starterGui = game:GetService("StarterGui")
    if starterGui then
        local sgChildren = starterGui:GetChildren()
        print("  📁 Enfants de StarterGui (" .. #sgChildren .. "):")
        for _, child in ipairs(sgChildren) do
            print("    📄 " .. child.Name .. " (" .. child.ClassName .. ")")
        end
    end
    
    -- Explorer les services
    print("\n⚙️ EXPLORATION DES SERVICES:")
    local services = {
        "Lighting",
        "SoundService", 
        "TweenService",
    ...
    for _, serviceName in ipairs(services) do
        ...
        if service then
            ...
            print("  📁 " .. serviceName .. " (" .. #serviceChildren .. " éléments)")
            
            -- Explorer les premiers éléments
            for i = 1, math.min(5, #serviceChildren) do
                local item = serviceChildren[i]
                print("    📄 " .. item.Name .. " (" .. item.ClassName .. ")")
            end
            
            if #serviceChildren > 5 then
                print("    📄 ... et " .. (#serviceChildren - 5) .. " autres éléments")
            end
        end
    end
    ...
    while true do
        ...
        if char and char:FindFirstChild("Humanoid") then
            ...
            if isGodModeActive then
                ...
                -- Protection supplémentaire contre les dégâts
                if char.Humanoid.Health < 999999 then
                    char.Humanoid.Health = 999999
                    print("🛡️ DEBUG: Santé restaurée à 999999")
                end
            end
        end
...
print("Modern Mod Menu V6 chargé! Exploration complète du jeu disponible")
