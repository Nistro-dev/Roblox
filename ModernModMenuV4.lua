...
function createGUI()
    ...
    screenGui.Name = "ModernModMenuV4"
...
-- Fonction pour appliquer le god mode avec protection renforcée
function applyGodMode()
    ...
    if char and char:FindFirstChild("Humanoid") then
        ...
        char.Humanoid.Immortal = true
        
        -- Protection contre les dégâts
        if char.Humanoid:FindFirstChild("HealthChanged") then
            char.Humanoid.HealthChanged:Connect(function()
                if isGodModeActive then
                    char.Humanoid.Health = math.huge
                    print("🛡️ DEBUG: Santé restaurée à l'infini")
                end
            end)
        end
        
        print("🛡️ God Mode appliqué avec protection renforcée")
    end
end
...
function showDebugInfo()
    ...
    if char and char:FindFirstChild("Humanoid") then
        ...
        print("  🛡️ Immortal: " .. tostring(humanoid.Immortal))
...
-- Boucle de force continue avec god mode renforcé
task.spawn(function()
    while true do
        local char = player.Character
        if char and char:FindFirstChild("Humanoid") then
            ...
            if isGodModeActive then
                ...
                if char.Humanoid.Health < math.huge then
                    char.Humanoid.Health = math.huge
                    print("🛡️ DEBUG: Santé forcée à l'infini")
                end
            end
        end
...
print("Modern Mod Menu V4 chargé! Vitesse jusqu'à 5000 + God Mode renforcé + Kill counter")
